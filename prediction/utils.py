...
def read_sample_fixed_season():
    file_path = "/scratch/project16/UShER/data/20241120/sample_fixed_1229_5aa.csv"
    result = {}
    ...
    with open(file_path, 'r') as csv_file:
        ...
        for row in csv_reader:
            position = row[1]  # The "Position" column
            values = row[2:]  # The rest of the columns
            result[position] = [v[2] for v in values if len(v)>= 3]
    return result

def select_trigram_with_sliding_window(position_string, trigram_idx_strings, last_label_list, limit):
    parsed_trigram_idxs = []
    time_list = []
    label_list = []
    position_season_dict = read_sample_fixed_season()
    for posId, example in zip(position_string, trigram_idx_strings):
        pos_label_list = position_season_dict[posId]
        new_example = []
        timestamps = []
        for idx, e in enumerate(example):
            if not pd.isna(e):
                new_example.append(ast.literal_eval(e))
                timestamps.append(idx)
        parsed_trigram_idxs.extend([new_example[i: i+limit] for i in range(len(new_example)-limit+1)])
        time_list.extend([timestamps[i: i+limit] for i in range(len(timestamps)-limit)])
        label_list.extend(pos_label_list[limit:])
    print(f"training with data size: {len(parsed_trigram_idxs)}, {len(label_list)}")
    return parsed_trigram_idxs, time_list, label_list

def select_esm_with_sliding_window(position_string, esm_embeds_list, last_label_list, limit):
    parsed_trigram_idxs = []
    time_list = []
    label_list = []
    position_season_dict = read_sample_fixed_season()
    for posId, example in zip(position_string, esm_embeds_list):
        pos_label_list = position_season_dict[posId]
        new_example = []
        timestamps = []
        for idx, e in enumerate(example):
            if not pd.isna(e):
                ...
                timestamps.append(idx)
        parsed_trigram_idxs.extend([new_example[i: i+limit] for i in range(len(new_example)-limit+1)])
        time_list.extend([timestamps[i: i+limit] for i in range(len(timestamps)-limit+1)])
        label_list.extend(pos_label_list[limit:])
    print(f"training with data size: {len(parsed_trigram_idxs)}, {len(label_list)}")
    return np.array(parsed_trigram_idxs), np.array(time_list), np.array(label_list)
...
def read_dataset_with_slide_window(path, data_path, concat=False):
    ...
    position_strings = df['Position'].values
    trigram_idx_strings = df.iloc[:, 3:].values # field: Position,predict_date,Label,2019-12, ... so start from third one
    parsed_trigram_idxs, time_list, label_list = select_trigram_with_sliding_window(position_strings,trigram_idx_strings, labels, limit=5)
    ...
    return trigram_vecs, label_list
    
def read_data_esm_with_slide_window(path):
    df = pd.read_csv(path)

    labels = df['Label'].values
    position_strings = df['Position']

    esm_embeds_list = df.iloc[:, 3:].values  # field: Position,predict_date,Label,2019-12, ... so start from third one
    esm_embeds_list, time_list, label_list = select_esm_with_sliding_window(position_strings, esm_embeds_list, labels, limit=5)

    esm_embeds_list = esm_embeds_list.transpose(1, 0, 2)

    return esm_embeds_list, label_list

def read_data_esm_with_slide_window_cat(path):
    df = pd.read_csv(path)
...
# y
def read_esm1v1_dataset(path):
    """
    Reads the data set from given path, expecting it to contain a 'y' column with
    the label and each year in its own column containing a number of site vector. 
    Each vector is a 1280 dimensional vector.
    Limit sets the maximum number of examples to read, zero meaning no limit.
    they are instead summed elementwise.
    """
    # subtype_flag, data_path = make_dataset.subtype_selection(subtype)
    ...
    labels = df['y'].values
    vector_strings = df.loc[:, df.columns != 'y'].values
    trigram_vecs = np.array([list(map(lambda x: ast.literal_eval(x), tempo_vectors)) for tempo_vectors in vector_strings])
...
def read_dataset(path, data_path, limit=0, concat=False):
    ...
    trigram_idx_strings = df.iloc[:, 1:8].values
