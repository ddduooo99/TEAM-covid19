...
class TERnnModel(nn.Module):
    """
    An RNN model using either RNN, LSTM or GRU cells with trainable time embeddings.
    """
    def __init__(self, input_dim, output_dim, hidden_size, dropout_p, cell_type, max_time_steps, time_embedding_dim):
        super(TERnnModel, self).__init__()
        ...
        # Adjust input dimension to account for concatenated time embeddings
        adjusted_input_dim = input_dim + time_embedding_dim
        
        if cell_type == 'LSTM':
            self.encoder = nn.LSTM(adjusted_input_dim, hidden_size)
        elif cell_type == 'GRU':
            self.encoder = nn.GRU(adjusted_input_dim, hidden_size)
        elif cell_type == 'RNN':
            self.encoder = nn.RNN(adjusted_input_dim, hidden_size)
        
        self.out = nn.Linear(hidden_size, output_dim)

    def forward(self, input_seq, time_indices, hidden_state):
        """
        Forward pass for the RNN model.

        Args:
        - input_seq: Tensor of shape [seq_len, batch_size, input_dim]
        - time_indices: Tensor of shape [seq_len, batch_size] containing time indices
        - hidden_state: Initial hidden state for the RNN

        Returns:
        - score_seq: Output scores of shape [batch_size, output_dim]
        - dummy_attn_weights: Placeholder for attention weights
        """
        # Obtain time embeddings
        time_embeddings = self.time_embedding(time_indices)  # [seq_len, batch_size, time_embedding_dim]
        
        # Concatenate input sequence with time embeddings
        input_with_time = torch.cat((input_seq, time_embeddings), dim=-1)  # [seq_len, batch_size, input_dim + time_embedding_dim]
        
        input_with_time = self.dropout(input_with_time)
        encoder_outputs, _ = self.encoder(input_with_time, hidden_state)
        score_seq = self.out(encoder_outputs[-1, :, :])  # Output for the last time step
    ...
    def init_hidden(self, batch_size):
        """
        Initialize hidden states for the RNN.

        Args:
        - batch_size: Size of the batch

        Returns:
        - Initialized hidden state(s)
        """
        if self.cell_type == 'LSTM':
            h_init = torch.zeros(1, batch_size, self.hidden_size)
            c_init = torch.zeros(1, batch_size, self.hidden_size)
